package com.github.binitabharati.kafkapoc.case7;
...
public class Producer {
	...
	private void sendMsg() throws Exception {
		...
		String[] recordKeys = new String[2];
		recordKeys[0] = "partition-0";
		recordKeys[1] = "partition-1";
		Random rand = new Random();
		for (int i = startIdx ; i < endIdx ; i++) {		
			String msg = "CHK-"+i;
			String recordKey = recordKeys[rand.nextInt(2)];
			...
			try {
				...
				logger.info("Sending message with key="+ recordKey + "-END-KEY" + ", msg = "+msg);				
				RecordMetadata metadata = (RecordMetadata)kafkaProd.send(pr).get();
				 if (metadata != null) {
					 	long tt2 = System.nanoTime() - startTime2;
	                    logger.info("sent record key=" + pr.key() + "-END-KEY" + ", msg = "+msg +
				...
				long tt2 = System.nanoTime() - startTime2;
				logger.info("Sending message with key  =  "+ recordKey + ", msg = "+msg + " caused Exception after waiting for response for time = "+TimeUnit.SECONDS.convert(tt2, TimeUnit.NANOSECONDS));		
