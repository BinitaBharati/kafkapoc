...
public class Producer {
	...
	private List<String> wikiTitlesList;
	...
	public Producer() {
		...
		wikiTitlesList = WikiRestClient.getWikiTitles("api");
	}
	...
	private void sendMsg() throws Exception {
		...
		for (int i = 0 ; i < 1000000 ; i++) {
			if (wikiTitleIdx >= wikiTitlesList.size()) {
				wikiTitleIdx = 0;
			}
			String msg = WikiRestClient.getWikiContent(wikiTitlesList.get(wikiTitleIdx));
			wikiTitleIdx++;
